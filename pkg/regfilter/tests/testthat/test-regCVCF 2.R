...
test_that("the result has the correct class", {
  ...
  data(rock)
  set.seed(9)
  out <- regCVCF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)])
...
test_that("Invalid threshold value", {
  ...
  # Default method
  expect_error(regCVCF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], t=2))
  # Class formula method
  expect_error(regCVCF(formula = perm ~ ., data = rock, t=2))
})
...
test_that("Invalid nfolds value", {
  ...
  # Default method
  expect_error(regCVCF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], nfolds=1))
  # Class formula method
  expect_error(regCVCF(formula = perm ~ ., data = rock, nfolds=1))
})
