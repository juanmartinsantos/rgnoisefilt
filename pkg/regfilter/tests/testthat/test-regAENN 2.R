...
test_that("Invalid threshold value", {
  ...
  # Default method
  expect_error(regAENN(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], t=2))
  # Class formula method
  expect_error(regAENN(formula = perm ~ ., data = rock, t=2))
})
...
test_that("Invalid k value", {
  ...
  # Default method
  expect_error(regAENN(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], k=0))
  # Class formula method
