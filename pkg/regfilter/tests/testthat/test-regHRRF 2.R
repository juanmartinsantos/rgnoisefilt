...
test_that("the result has the correct class", {
  ...
  data(rock)
  set.seed(9)
  out <- regHRRF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)])
...
test_that("Invalid threshold value", {
  ...
  # Default method
  expect_error(regHRRF(x = rock[,-ncol(rock)], y = rock[,ncol(rock)], t=2))
  # Class formula method
  expect_error(regHRRF(formula = perm ~ ., data = rock, t=2))
})
